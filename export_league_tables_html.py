...
OUTPUT (into --outdir, "league_html" in the examples above):
  index.html            season hub: sponsor strip, links to every table
                         (no on-page <h1> — see INDEX HEADING below)
...
INDEX HEADING (changed 2026-09-23): index.html no longer renders an <h1>.
It's embedded in a WordPress page whose own title already provides the
heading, so the in-page <h1> just duplicated it. --site-title now only sets
the browser-tab <title>. Table pages still keep their own <h1> (the table
name), since nothing else on the page says which table you're looking at.

FOOTER (removed 2026-09-23, Adam's request): index.html and table pages no
longer end with a "WMCCL League Tables — generated by
export_league_tables_html.py" <footer>. The `.wmccl-league footer` CSS rule
is left in SITE_CSS (unused, harmless) so CSS_VERSION doesn't change just
for this.
...
CSS. Three independent fixes, each covering the others' gaps:
  ...
  2. .htaccess (HTACCESS_CONTENT / write_htaccess()): sends
     `Cache-Control: no-cache` for .html/.css/.json/.pdf — meaning
     "revalidate before reuse", NOT "don't cache": an unchanged file costs
     a tiny 304 Not Modified, a changed one is fetched fresh. Sponsor
     images get a 1-week cache. Only takes effect on Apache/LiteSpeed hosts
     with mod_headers (the <IfModule> guard makes it a harmless no-op
     ...
     exclude this folder. (.pdf added 2026-09-23.)
  3. Page-link cache-busting (added 2026-09-23, Adam's request — pages
     inside the WordPress <iframe> were still showing stale copies, which
     means fix 2 isn't taking effect on the live host): every link BETWEEN
     pages (index -> table, table -> rider, table -> index, award ->
     index, rider -> award/index) goes through vurl(), which appends
     `?v=<BUILD_VERSION>`. BUILD_VERSION is a timestamp taken when the
     script runs (or the WMCCL_BUILD_VERSION environment variable, if set,
     so a .sh wrapper can make every script in one update agree). A
     weekly re-run therefore gives every page a brand-new URL, which the
     browser has never cached, so it has to fetch it. The index page
     itself is the one URL that can't be busted from in here — the
     WordPress Custom HTML block handles that by setting the <iframe>
     src to `index.html?v=` + Date.now() from a tiny inline script.
     export_team_awards_html.py imports vurl from here;
     export_rider_pages.py (standalone, no import from here) carries its
     own identical copy.
...
def standings_line(upto_round: int, rounds: int, rounds_info: Dict[int, Dict[str, str]]) -> str:
    """
    The "Standings after round X of Y" line shown near the top of every table
    page and index.html (added 2026-09-23 — the PDFs already show the same
    line). Adds the venue/date of round X when the rounds table has them,
    e.g. "Standings after round 3 of 11 (Sutton Park, 2026-10-04)".
    Returns "" before any round has results, so no line is shown then.
    """
    if not upto_round or upto_round < 1:
        return ""
    text = f"Standings after round {upto_round} of {rounds}"
    info = rounds_info.get(upto_round) or {}
    extra = ", ".join(x for x in (info.get("venue", ""), info.get("date", "")) if x)
    if extra:
        text += f" ({extra})"
    return f'<p class="standings">{esc(text)}</p>'
...
.wmccl-league .standings { font-weight: 600; margin: 0.3rem 0; }
...
# Cache-busting version for links between pages (see BROWSER CACHING fix 3
# in the module docstring). A timestamp of this run, so every weekly update
# gives every page a URL the browser has never seen before. Set
# WMCCL_BUILD_VERSION in the environment to make several scripts in one
# update share the same value (not required — any value that changes each
# run works).
...
#
# Pages, CSS, JSON and PDFs: browsers must check with the server before
# reusing a cached copy ("no-cache" = revalidate, not "never cache"). An
# unchanged file costs a tiny "304 Not Modified"; a changed one is fetched
# fresh, so riders always see this week's tables without needing to refresh.
<IfModule mod_headers.c>
  <FilesMatch "\\.(html?|css|json|pdf)$">
...
def render_table_page(table_name: str, profile: str, rows: List[Dict],
                      ...
                      sponsors: List[Dict[str, str]], upto_round: int = 0) -> str:
    sponsor_html = render_sponsor_strip(sponsors, "../")
    standings_html = standings_line(upto_round, rounds, rounds_info)
    round_headers = []
    for rnd in range(1, rounds + 1):
    ...
    for r in rows:
        ...
        rider_href = vurl(f"../riders/{r['race_number']}.html")
        name_link = f'<a href="{esc(rider_href)}">{esc(r["name"])}</a>'
...
def render_index_page(manifest: Dict[str, Dict],
                      ...
                      rounds_info: Optional[Dict[int, Dict[str, str]]] = None) -> str:
    sponsor_html = render_sponsor_strip(sponsors, "")

    # "Standings after round X of Y" for the whole site. Each manifest entry
    # records the upto_round/rounds its own DB run used; the index shows the
    # latest round across all of them (normally every DB is at the same
    # round after a full weekly update). Entries written before 2026-09-23
    # don't have these keys — they're simply ignored until that profile is
    # next re-run, and if none have them the line is left off.
    uptos = [m["upto_round"] for m in manifest.values() if m.get("upto_round")]
    totals = [m["rounds"] for m in manifest.values() if m.get("rounds")]
    standings_html = standings_line(max(uptos), max(totals), rounds_info or {}) if uptos and totals else ""
    ...
    # Adam's request). The page's own <h1> was then dropped as well
    # (2026-09-23, Adam's request) — the WordPress page embedding this
    # index already shows its own title, so site_title is now only used for
    ...
    for profile in PROFILE_ORDER:
        ...
        links = "".join(
            f'<li><a href="{esc(vurl(f"tables/{name}.html"))}">{esc(name)} ({meta.get("riders", 0)})</a></li>'
    ...
    # The cache stores plain hrefs; ?v= is added here at render time.
    awards_html = ""
    if awards:
        award_links = "".join(
            f'<li><a href="{esc(vurl(a["href"]))}">{esc(a["label"])}</a></li>'
            for a in awards
        )
...
def main():
    ...
    ap.add_argument("--site-title", default="WMCCL League Tables",
                    help="Browser-tab <title> for index.html (no on-page heading; the WordPress page supplies it)")
    ...
    print(f"Output dir: {outdir}  (css version {CSS_VERSION}, build version {BUILD_VERSION})\n")
    ...
    for table_name, cat_list in tables.items():
        ...
        page_html = render_table_page(table_name, args.profile, rows, args.rounds, args.best,
                                      rounds_info_for_index, sponsors_cache, upto_round=upto_round)
    ...
    index_html = render_index_page(manifest, sponsors_cache, args.site_title, awards=awards_cache,
                                   rounds_info=rounds_info_for_index)
